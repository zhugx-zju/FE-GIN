@echo off
REM Reproducibility 实验完整运行脚本（Windows版）
...
echo.
echo 步骤 1/3: 检查数据集...
if not exist "data\data_bil\force_load" (
    echo ❌ 错误：源数据集不存在！
    echo.
    echo 请先下载数据集：
...
if not exist "data\data_exp\force_load" (
    echo ❌ 错误：data_exp 数据集不存在！
    pause
    exit /b 1
)
if not exist "data\data_grf\force_load" (
    echo ❌ 错误：data_grf 数据集不存在！
    pause
    exit /b 1
)
echo ✓ 源数据集存在
...
if not exist "data\data_mix\force_load\train" (
...
echo.
set /p CONFIRM="是否开始运行？这可能需要几小时... (Y/N): "
if /i "%CONFIRM%"=="Y" (
    cd igfe_unet\script
